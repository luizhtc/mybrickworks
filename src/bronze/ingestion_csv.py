...
csv_file_path = os.path.join("/Volumes/cat_olist/sch_bronze/vol_landing/olist/", csv_file_name)
df = (
    spark
        .read
        .option("header", True)
...
df.write.format("delta").mode("overwrite").saveAsTable(f"`cat_olist`.`sch_bronze`.`{table_name}`")
