...
from pyspark.sql.functions import col, when, countDistinct, avg, sum as _sum, round as _round
...
orders = (
    spark
        .read
        .table("cat_olist.sch_silver.orders")
...
orders_reviews.withColumn(
    ...
    _round((_sum(when(col("review_score") <= 2, 1).otherwise(0)) / countDistinct(col("order_id")) * 100), 2).alias("pct_negative_reviews"),
).display()
