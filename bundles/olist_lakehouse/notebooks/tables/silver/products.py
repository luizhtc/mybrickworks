...
products_calc = products_join.withColumn(
    "product_volume_cm3",
    col("product_length_cm").cast("int") * col("product_height_cm").cast("int") * col("product_width_cm").cast("int"),
...
(products_transformed.write.format("delta").mode("overwrite").saveAsTable("`cat_olist`.`sch_silver`.`products`"))
